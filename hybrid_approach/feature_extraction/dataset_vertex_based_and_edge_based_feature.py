...
def features_h5_per_sample(ddacs, out_dir, save_or_read):
    if save_or_read == "save":
        ...
        for i in tqdm(range(n), desc="Saving features (HDF5 per sample)"):
            ...
            new_concatenated_features, node_displacement = prepare_sample(h5_path)
            new_concatenated_features = new_concatenated_features.astype(np.float32)
            #print(f"new_concatenated_features of nodes: {X}")
            node_displacement = node_displacement.astype(np.float32)

            with h5py.File(out_file, "w") as f:
                f.create_dataset("new_concatenated_features", data=new_concatenated_features)
                f.create_dataset("node_displacement", data=node_displacement)
    ...
    elif save_or_read == "read":
        out_file = out_dir / f"16040.h5"
        with h5py.File(out_file, "r") as f:
            new_concatenated_features = f["new_concatenated_features"][:]   # (N, 34) float32
            node_displacement = f["node_displacement"][:]          # (N, 3)  float32
            sample_id = f.attrs["sample_id"]
            print(f"For sample {sample_id}, its new_concatenated_features :\n {new_concatenated_features[0]}\n its node_displacement:\n {node_displacement[0]}")
            
    else:
        print(r"U should choose save or read")

# Run it
features_h5_per_sample(dataset, OUT_DIR, save_or_read = "save")