...
# 1) import to register encoders/loaders/etc.
import grit  # noqa
...
def infer_one_dir(data_dir,
                ...
                cfg_path,
                out_dir,
                allow_missing_y=True,
                device='cuda:0'):
    # load cfg and lock key settings for inference
    set_cfg(cfg) 
    cfg.set_new_allowed(True)
    #load_cfg(cfg, argparse.Namespace(cfg_file=cfg_path))
    load_cfg(cfg, Namespace(cfg_file=cfg_path, opts=[]))
    cfg.device = device
    ...
    # 4) build a simple PyG DataLoader over the folder
    ds = DDACSNPYStream(root=data_dir, debug=False) # allow_missing_y=allow_missing_y)
